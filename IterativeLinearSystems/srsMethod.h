void SRSMethod(unsigned int numberOfRows, double tolerance, double matrix[numberOfRows][numberOfRows + 1], double solution[numberOfRows], unsigned int numberMaxOfIterations, float relaxationFactor)
{
    ...
    do
    {
        ...
        for (unsigned int i = 0; i < numberOfRows; i++)
        {
            ...
            solution[i] = (1 - relaxationFactor) * solution[i] + relaxationFactor * ((matrix[i][numberOfRows] - sumOfLine) / matrix[i][i]);
        }
