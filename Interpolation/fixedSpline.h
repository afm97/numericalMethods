float FixedSpline(int rows, int cols, float data[rows][cols], float constants[rows][3])
{
    ...
    float h_0, h_1, h_n, a_0, a_1, a_2;

    h_0 = data[1][0] - data[0][0];
    h_n = data[rows - 1][0] - data[rows - 2][0];

    A[0][0] = 2 * h_0;
    A[0][1] = h_0;
    A[rows - 1][rows - 1] = 2 * h_n;
    A[rows - 1][rows - 2] = h_n;

    constants[0][1] = (3 / h_0) * (data[1][1] - data[0][1]) - 3 * data[0][2];
    constants[rows - 1][1] = 3 * data[rows - 1][2] - (3 / h_n) * (data[rows - 1][1] - data[rows - 2][1]);
